...
export default function SeriesDaemonCard({ series, index, onOpen }: Props) {
  ...
  return (
    <div ref={ref} onClick={() => onOpen(series)} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ position: 'relative', cursor: 'pointer', opacity: 0, background: hovered ? '#141414' : '#101010', border: `1.5px solid ${hovered ? c : '#243028'}`, borderRadius: 10, padding: '20px 22px', overflow: 'hidden', transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)', transform: hovered ? 'translateY(-3px)' : 'none', boxShadow: hovered ? `0 10px 32px rgba(0,0,0,0.45), 0 0 22px ${c}22` : '0 2px 8px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column', gap: 14, minHeight: 320 }}>
      ...
      <div style={{ position: 'relative', zIndex: 3 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          <span style={{ fontFamily: 'var(--hf-mono)', fontSize: 12, color: '#4a6a55' }}>~/research/</span>
          <GlitchText as="h3" intensity={hovered ? 'medium' : 'low'} style={{ fontFamily: 'var(--hf-display)', fontSize: 21, color: '#e4ecd8', lineHeight: 1.1 }}>{series.title}</GlitchText>
        </div>
        <p style={{ fontFamily: 'var(--hf-mono)', fontSize: 11.5, color: '#7a9a88', lineHeight: 1.6, marginTop: 8 }}>{series.blurb}</p>
      </div>
      <div style={{ position: 'relative', zIndex: 3, display: 'flex', alignItems: 'flex-end', gap: 12, marginTop: 'auto' }}>
        ...
        <span style={{ fontFamily: 'var(--hf-mono)', fontSize: 11, color: '#7a9a88', fontWeight: 600 }}>{series.editions.length}<span style={{ color: '#4a6a55', fontWeight: 400 }}> {series.editions.length === 1 ? 'edition' : 'editions'}</span></span>
      </div>
      <div style={{ position: 'relative', zIndex: 3, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, borderTop: '1px solid #243028', paddingTop: 12 }}>
        <div>
          <div style={{ fontFamily: 'var(--hf-mono)', fontSize: 9, color: '#4a6a55', letterSpacing: '0.1em', marginBottom: 3 }}>LAST RUN</div>
          <div style={{ fontFamily: 'var(--hf-mono)', fontSize: 11, color: '#e4ecd8' }}>{latest ? <>{latest.week} <span style={{ color: '#6dbf8b' }}>✓</span></> : <span style={{ color: '#4a6a55' }}>— never —</span>}</div>
        </div>
        <div>
          <div style={{ fontFamily: 'var(--hf-mono)', fontSize: 9, color: '#4a6a55', letterSpacing: '0.1em', marginBottom: 3 }}>NEXT RUN</div>
