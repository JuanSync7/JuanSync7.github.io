...
export default function BlogCard({ post, index, layout, onClick }: Props) {
  ...
  return (
    <div
      ...
      style={{
        ...span(), position: 'relative', background: hovered ? palette.surface2 : palette.surface,
        border: `1.5px solid ${hovered ? c : palette.line}`, borderRadius: 8,
        ...
        boxShadow: hovered ? `0 8px 32px ${alpha(palette.black, 0.4)}, 0 0 20px ${alpha(c, 0.133)}` : `0 2px 8px ${alpha(palette.black, 0.2)}`,
    ...
    >
      {big ? (
        <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(180deg, ${alpha(palette.bg2, 0.5)} 0%, ${alpha(palette.bg2, 0.92)} 55%, ${palette.bg2} 100%)` }} />
        </div>
      ) : (
      ...
      <div style={{ position: 'relative', zIndex: 3 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          {post.featured && (
            <span style={{ fontSize: 9, fontFamily: 'var(--hf-mono)', color: palette.magenta, padding: '2px 6px', border: `1px solid ${alpha(palette.magenta, 0.267)}`, borderRadius: 999, letterSpacing: '0.08em', textTransform: 'uppercase', background: alpha(palette.magenta, 0.067) }}>featured</span>
          )}
        </div>
        <GlitchText as="h3" intensity={hovered ? 'medium' : 'low'} style={{ fontFamily: isLarge ? 'var(--hf-display)' : 'var(--hf-mono)', fontSize: isLarge ? 24 : post.size === 'medium' ? 18 : 15, color: palette.ink, lineHeight: 1.3, marginBottom: 8, fontWeight: isLarge ? 400 : 600 }}>{post.title}</GlitchText>
        {big && <p style={{ fontFamily: 'var(--hf-mono)', fontSize: 12, color: palette.inkSoft, lineHeight: 1.6, margin: 0 }}>{post.excerpt}</p>}
      </div>

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16, position: 'relative', zIndex: 3, borderTop: `1px solid ${palette.line}`, paddingTop: 12 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <span style={{ fontFamily: 'var(--hf-mono)', fontSize: 10, color: palette.inkMuted, letterSpacing: '0.04em' }}>{new Date(post.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
          <span style={{ fontFamily: 'var(--hf-mono)', fontSize: 10, color: palette.inkMuted }}>{post.readTime}</span>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {post.tags.slice(0, 2).map((t) => (
            <span key={t} style={{ fontFamily: 'var(--hf-mono)', fontSize: 9, color: palette.inkMuted, padding: '1px 6px', border: `1px solid ${alpha(palette.line, 0.267)}`, borderRadius: 3, letterSpacing: '0.04em' }}>{t}</span>
          ))}
        </div>
      </div>
