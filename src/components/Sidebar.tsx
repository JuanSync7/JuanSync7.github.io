...
import { Github, Linkedin, Mail, BookOpen, Code, FileText, Terminal, Sun, Moon } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeSection, onNavigate, toggleTheme, isDark }) => {
  const navItems = [
    ...
    <aside className="w-full md:w-64 md:fixed md:h-screen bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-colors duration-300 z-50">
      <div className="p-8 flex flex-col items-center text-center border-b border-slate-100 dark:border-slate-800">
        <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-slate-100 dark:border-slate-800 mb-4 shadow-lg group relative">
            ...
            <img 
              src="https://github.com/JuanSync7.png" 
              alt="Profile" 
              className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-500" 
              onError={(e) => { (e.target as HTMLImageElement).src = 'https://api.dicebear.com/7.x/avataaars/svg?seed=Juan' }} 
            />
        </div>
        ...
        <div className="flex gap-3 justify-center">
          <a href="https://github.com/JuanSync7" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors"><Github size={20} /></a>
          <a href="https://www.linkedin.com/in/shewjuankok/" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-blue-600 transition-colors"><Linkedin size={20} /></a>
